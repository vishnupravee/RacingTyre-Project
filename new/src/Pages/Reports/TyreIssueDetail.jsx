...
import { getScanningTyreSubReportguid, getTyreIssueReportList, getTyreScanningReportList, getTyreSubReportguid } from "../../core/services/ReportApiServices";
...
const TyreIssueDetail = () => {
    ...
    const handleTyreReportList = async () => {
        ...
        try {
            ...
            if (Array.isArray(response.listTyreVerificationReport)) {
                const filteredData = response.listTyreVerificationReport?.filter((entry) => {
                  ...
                  const matchesName = fullname
                    ? entry.racerName?.toLowerCase().includes(fullname.toLowerCase())
                    : true;
  ...
  const handleVeiwClick = async (guid) => {
    ...
    const tyreColumns = Array.from({ length: 4 }, (_, i) => ({
        title: `Tyre ${i + 1}`,
        dataIndex: `tyre${i + 1}`,
    }));
...
export default TyreIssueDetail;