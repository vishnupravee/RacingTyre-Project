...
const TyreReports = () => {
  ...
  const handleTyreReportList = async () => {
    ...
    try {
      const response = await getTyreIssueReportList(selectedCategory, fullname, racerNo);
      if (Array.isArray(response.listTyreReport)) {
        const sortedEvents = response.listTyreReport.sort((a, b) =>
          sortDirection === "asc"
