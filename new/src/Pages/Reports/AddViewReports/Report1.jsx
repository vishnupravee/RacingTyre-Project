...
const Report1 = ({ data = [], handleClose }) => {
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(16);
    doc.text("Tyre Issue Report", 20, y);
    y += 20; // Add some space after the title

    data.forEach((val, index) => {
      // Check if we need to add a new page before adding the new entry
      if (y + 60 > pageHeight) {
      ...
      // Adding Tyre details (if any), excluding 'tyreGuId'
      Object.keys(val)
      ...
      y += 5; // Add some space before the next entry
    });
                          ...
                          {data &&
                            ...
                            data.map((val, index) => (
                              <div key={index} className="racer-info">
                                ...
                                {/* <table className="table table-bordered table-dark table-hover table-striped align-middle text-center">
  ...
  <tbody>
    {Array.from({ length: 16 }, (_, i) => (
      <tr key={i}>
        <td className="fw-bold">{`Tyre ${i * 2 + 1} = ${val[`tyre${i * 2 + 1}`] || 'N/A'}`}</td>
        <td className="fw-bold">{`Tyre ${i * 2 + 2} = ${val[`tyre${i * 2 + 2}`] || 'N/A'}`}</td>
      </tr>
    ))}
  </tbody>
</table> */}
