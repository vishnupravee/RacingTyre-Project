...
import EventModal from "../../MasterModal/EventModal";
import { deleteEvent, getEventguid, getEventList } from "../../core/services/MasterApiServices";
...
const toggleSortOrder = () => {
  ...
  const handleEventList = async () => {
    try {
      ...
      if (Array.isArray(response.listEvents)) {
        const sortedEvents = response.listEvents.sort((a, b) =>
          sortDirection === "asc"
  ...
  const handleEditClick = async (guid) => {
    try {
      const response = await getEventguid(guid);
      console.log(response,"res");
      
      setUpdateData(response.events); // Set the data for editing
  ...
  const handleDeleteClick = (guid) => {
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await deleteEvent(guid);
          if (response) {
            MySwal.fire({
              title: "Deleted!",
              ...
              handleEventList(); // Refresh the list
  ...
  console.log(updateData,"eventtttt");
  const columns = [
    {
      title: "Name",
      dataIndex: "name",
